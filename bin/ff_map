...
def main():
    ...
    file_group.add_argument('-o', dest='outpath', type=str, required=True,
    ...
    # Hack to get GROMACS-style boolean flag behavior (argparse.BooleanOptionalAction
    #  only works if flags have double leading dashes). The default to 'cgsorder'
    #  is the way to get that string to show up in the -h list, but it always gets
    #  overridden later.
    mapping_group.add_argument('-cgsorder', dest='cgsorder', action='store_true', help=argparse.SUPPRESS)
    mapping_group.add_argument('-nocgsorder', dest='cgsorder', action='store_false', help=argparse.SUPPRESS)
    mapping_group.add_argument('-[no]cgsorder', dest='cgsorder', action='store_false',
                               help='Whether cgsmiles strings passed with -cgs or -cgsl are in the same order as -mols')
    mapping_group.set_defaults(cgsorder='assume yes if passed only with -cgs and no if -cgsl is used')
    mapping_group.add_argument('-lib', dest='library', help='loads library files', default=None)

    args = parser.parse_args()
 
    # to replace the cgsorder default string hack
    if not isinstance(args.cgsorder, bool):
        args.cgsorder = not args.cgsmiles_library

    print("INFO - Loading universe")
    # load trajectory (bonds are implicitly guessed, if missing from the topology)
    ...
    if args.map_file:
        ...
        mapped_atoms, bead_idxs, weights = forward_map_indices(init_universe,
                                                               mappings)
        cgsmiles_strs = None
        res_iter = None
    ...
    # these are all the multiframe writes MDAnalysis has
    mda_traj_formats = [fmt.casefold() for fmt in mda._MULTIFRAME_WRITERS.keys()]
    outpath = pathlib.Path(args.outpath)
    outpath_extension = outpath.suffix.casefold()[1:]
    single_frame_outpath = outpath
    # we have more than a single frame
    if cg_universe.trajectory.n_frames > 1:
        if outpath_extension not in mda_traj_formats:
            msg = (f"Requested output format {outpath_extension} is not known to MDAnalyis "
                    "or is not a able to write multiple frames.")
        ...
        single_frame_outpath = outpath.stem+'.gro'

    cg_beads = cg_universe.atoms
    # write first frame or just single frame
    with mda.Writer(single_frame_outpath,
                    multiframe=False,
