...
from fast_forward.interaction_distribution import interaction_distribution, RECOGNISED_INTERACTIONS, VS_TYPES
...
from fast_forward.constraint_coupling import report_constraint_coupling
...
def virtualsite_atoms(block):
    ...
    constructors = []
    for interaction_type in VS_TYPES:
...
def main():
    ...
    parser.add_argument('-plots', default=None, const=".", nargs="?",
                        dest="plots",
                        help="Plot the reference and fits of bonded interactions. Optionally provide a path (default: current dir).")
    ...
    parser.add_argument('-interactions', dest="int_mode", choices=["guess", "comments", "all"],
                        default="comments",
                        help="Interactions (angles and dihedrals) to fit. Will use only commented interactions in itp by default, or all interactions if 'all' is specified. Will guess interactions from the bonds if 'guess' is specified.")
    ...
    if args.itp_files:
        ...
        for molname, block in ff.blocks.items():
            ...
            if args.int_mode == 'guess':
                guess_interactions(block)
            interaction_groups, initial_parameters, block_indices = interaction_mapper.get_interactions_group(molname, args.int_mode)

            fitter.dihedrals = interaction_groups # save dihedrals in fitter to later check if angles are part of dihedrals

            if not interaction_groups:
                print(f"WARNING: No interactions found in block {molname}. None will be fitted for this molecule.")
                if args.int_mode == 'comments':
                    print("\t Add them by commenting relevant lines in the itp file, use -interactions all, or -interactions guess.")
                elif args.int_mode == 'all':
                    print("\t Add them to the itp file, or use interactions guess.")
                else:
                    print("\t Check the formatting of the itp file.")
                continue
            for inter_name in RECOGNISED_INTERACTIONS:
                for (group_name, pair_idxs), (parameter_type) in zip(interaction_groups[inter_name].items(), initial_parameters[inter_name].values()):


                    distr, inter_type = interaction_distribution(u, inter_name, pair_idxs, group_name,
                                                     args.prefix, args.distribution_data,
                                                     parameter_type)


                    # fit the interactions
                    fitter.fit_interaction(distr, block_indices[inter_name][group_name], group_name, inter_type,
                                           vs_constructors)

                    if 'virtual' in inter_name:
            ...
            if args.plots:
                make_distribution_plot(fitter.plot_parameters,
                                       args.plot_data, name =f'{args.plots}/fitted_interactions')
